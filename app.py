...
st.markdown("""
    ...
    .quota-box { text-align: center; background-color: #e8f5e9; padding: 25px; border-radius: 12px; border: 2px solid #c8e6c9; }
    .quota-valore { font-size: 36px; font-weight: 800; color: #1b5e20; }
...
if scelta == "🔍 Verifica Vincita":
    ...
    if st.button("VERIFICA ORA 🚀", type="primary", use_container_width=True):
        ...
        if vincite:
            ...
            st.markdown(f'<a href="https://wa.me/?text={urllib.parse.quote(testo_wa)}" target="_blank" class="wa-button">📲 PASSO 3: WhatsApp</a>', unsafe_allow_html=True)
        else:
