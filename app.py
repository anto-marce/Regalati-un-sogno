...
if 'numeri_estratti' not in st.session_state:
    st.session_state.numeri_estratti = [1, 2, 3, 4, 5, 6]
...
if scelta == "🔍 Verifica Vincite":
    ...
    # INPUT TESTUALE
    input_testo = st.text_input(
        "Incolla gli estratti qui (separati da uno spazio o un trattino) e premi INVIO:", 
        placeholder="Esempio: 10-22-35-44-51-68"
    )
    
    # Processamento input testuale
    if input_testo:
        trovati = re.findall(r'\d+', input_testo)
        if len(trovati) >= 6:
            st.session_state.numeri_estratti = [int(n) for n in trovati[:6]]
            st.success("✅ Numeri acquisiti correttamente!")

    # GRIGLIA NUMERI (Legge dallo stato della sessione)
    st.write("Conferma o modifica i numeri rilevati:")
    cols = st.columns(6)
    finali = []
    for i in range(6):
        n_val = cols[i].number_input(f"{i+1}°", 1, 90, value=st.session_state.numeri_estratti[i], key=f"input_manuale_{i}")
        finali.append(n_val)
    ...
    if conc_fatti == 0:
        st.markdown(f'<div class="status-blue">🚀 PRONTI AL VIA!<br><small>Tutti i 15 concorsi disponibili</small></div>', unsafe_allow_html=True)
    elif rimanenti <= 3:
    ...
    for i, s in enumerate(soci):
        with c1 if i < 3 else c2: st.checkbox(f"Pagato da {s}", key=f"paga_{s}")
    
    pagati = sum([st.session_state.get(f"paga_{s}", False) for s in soci])
    if pagati < 6:
    ...
    lordo = st.number_input("Vincita Lorda Totale (€)", min_value=0.0, step=0.01)
    if lordo > 0:
